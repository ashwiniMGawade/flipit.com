...
sub vcl_fetch {
    ...
    # Before you blindly enable this, have a read here: http://mattiasgeniar.be/2012/11/28/stop-caching-static-files/
    if (req.url ~ "^[^?]*\.(bmp|bz2|doc|eot|flv|gif|gz|ico|jpeg|jpg|less|mp[34]|pdf|png|rar|rtf|swf|tar|tgz|txt|wav|woff|zip)(\?.*)?$") {
        set beresp.ttl = 365d;
        set beresp.http.Cache-Control = "max-age = 31536000";
        unset beresp.http.set-cookie;
    }

    if (beresp.status >= 500 || beresp.status == 301 || beresp.status == 302 || beresp.http.X-Nocache  == "no-cache" || (req.url ~ "store/followbutton" && req.http.X-Varnish-Hashed-On ) || (req.url ~ "^[^?]*\.xml?$")) {
        set beresp.ttl = 0s;
